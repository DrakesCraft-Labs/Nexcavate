            ...
            for (SubCommand subcmd : subcommands) {
                if (args[0].equalsIgnoreCase(subcmd.getCommandName())) {
                    if (sender.hasPermission("nexcavate.command."+subcmd.getCommandName())) {
                        ...
                        sender.sendMessage("No tienes permiso para utilizar esta directiva.");
                        return false;
                    }
                }
            }
            sender.sendMessage("¡Subcomando desconocido! Los subcomandos disponibles son: " + subcommands.stream().map(SubCommand::getCommandName).collect(Collectors.joining(", ")));
            return false;
        }
        sender.sendMessage("Civilización versión renacentista:" + Nexcavate.instance().getDescription().getVersion());
        return true;
    }
