...
public class StructureScreenHandler implements NEGUIInventoryHolder {
    ...
    public void refresh() {
        inventory.setItem(0, new CustomItemStack(Material.ARROW, "&fVolver"));
        ...
        for (int i = size * 9 - 1; i > 0; i -= 9) {
            ...
            if (layernum - 1 == layer) {
                ...
                im.setDisplayName(im.getDisplayName() + ChatColor.GRAY + " (capa actual)");
    ...
    private ItemStack getDisplay(StructurePiece structurePiece) {
        ...
        if (!res.getType().isBlock()) {
            ...
            lore.add("");
            lore.add(Utils.color("&fColoca aquí un &esoporte para objetos"));
            lore.add(Utils.color("&fy mete el objeto en el soporte para ensamblarlo"));
        }
